  ...
  async handler(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      if (req.file) {
        ...
        const { ext, mime } = await fileTypeFromFile(path);
        ...
        const fileTypes = /pdf/;
        const _isPdf = fileTypes.test(extname(req.file.originalname).toLowerCase());
        ...
        if (_isPdf) {
          ...
          await video.fnExtractFrameToJPG(resolve(FILE_PATH), {
            number: 1,
            file_name,
          });
